<?
	...
	$to = "ludmilaTilahunGDC@i.ua, leads@ms-digital.top"; // Замениь на емаил клиента
	...
	$headers .= "From: no-reply@gvozdeva.dance"; // Заменить домен на домен клиента

	if (!$title && !$phonenum) {
	} else {
		mail($to,"New lead(gvozdeva.dance)",$message,$headers); // Заменить домен на домен клиента
	}
?>